...
export default async function Post() {
    ...
    return (
        ...
        <header className= "lg:mx-[162px] dark:bg-transparent bg-white">
            ...
            <hr className="border-[#F7AB0A] my-10 dark:bg-white bg-black dark:h-0 h-1"/>
            </header>
            ...
            {categories.map((category :Props) => {
                return (
                    <Link href={`/categories/category?category=${category.title}`} key={category.title} className="text-center text-md dark:text-[#F7AB0A] font-medium text-black hover:text-pink-500">{category.title}</Link>)}
                    )}
            </div>
            </div>
            <footer className="">
