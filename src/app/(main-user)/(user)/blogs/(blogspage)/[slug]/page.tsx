...
async function Post({ params: { slug } }: Props) {
  ...
  const prevPostSlug =
    (await client.fetch(prevQuery, { createdAt: post._createdAt })) !== null
      ? await client.fetch(prevQuery, { createdAt: post._createdAt })
      : "";
  const nextPostSlug =
    (await client.fetch(nextQuery, { createdAt: post._createdAt })) !== null
      ? await client.fetch(nextQuery, { createdAt: post._createdAt })
      : "";
      ...
      <header className="lg:mx-[150px]">
        ...
        <div className="my-10 justify-start text-green-400 flex hover:opacity-80 -mb-10">
          <Link
            href="/blogs"
            as="/blogs"
            className="inline-flex items-center px-3 lg:py-2 pr-5 text-md font-medium text-center"
          >
            Back to blogs
            <svg
              aria-hidden="true"
