  ...
  return (
    ...
    <div key={post._id} className="flex lg:flex-row md:flex-row flex-col flex-start lg:gap-32 md:gap-32">
                    ...
                    <div className="flex flex-col justify-center mx-4 my-5">
                        <Link href={`blogs/${post.slug.current}`} className="text-3xl dark:text-white font-bold md:text-3xl">{post.title}</Link>
                        <div className="flex gap-2">
                        {post.categories.map(tag=>{
                            return(
                                <Link href={`/categories/category?category=${tag.title}`} key={tag._id} className="dark:text-green-300 text-emerald-500">{tag.title}</Link>
                                )
                            })}
                        </div>
                    </div>
